...
export default function StagingPage() {
  ...
  const config = scanConfigs.staging;
  ...
  const preScannedEntries: ScannedParcelEntry[] = useMemo(
    () =>
      routeParcels
        .filter((p) => p.status === ParcelStatus.Staged)
        .map((p) => ({
          trackingNumber: p.trackingNumber,
          status: "success",
          previousStatus: "SORTED",
          newStatus: "STAGED",
          scannedAt: new Date(),
        })),
    [routeParcels]
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Staging Scan</h1>
        <p className="text-muted-foreground">
          Scan parcels with status <strong>Sorted</strong> to transition them to <strong>Staged</strong> for a selected route.
        </p>
      </div>
      ...
      {routeId && route && (
        <RouteParcelsTable parcels={routeParcels} targetStatus={ParcelStatus.Staged} />
      )}
    </div>
  );
}

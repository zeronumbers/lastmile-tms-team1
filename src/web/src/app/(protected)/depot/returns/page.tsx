...
export default function ReturnsPage() {
  ...
  const config = scanConfigs.returns;
  ...
  const handleScan = useCallback(
    async (trackingNumber: string): Promise<ScanResult> => {
      const result = await scanParcelMutation.mutateAsync({
        trackingNumber,
        newStatus: ParcelStatus.ReturnedToDepot,
      });
      return {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Returns Scan</h1>
        <p className="text-muted-foreground">
          Scan parcels with status <strong>Failed Attempt</strong> to transition them to <strong>Returned to Depot</strong>.
        </p>
      </div>
